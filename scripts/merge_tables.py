    ...
    for f in aaastrIn:
        with open(f) as csvfile:
            ...
            for astrLine in iIn:
                ...
                if strID not in hashIDs:
                    hashIDs[strID] = iLine
                    aastrData.append(astrData)
                else:
                    aastrData[hashIDs[strID]] = [ str(int(aastrData[hashIDs[strID]][i])+int(astrData[i])) for i in range(len(astrData)) ]
                    aastrData.append(aastrData[hashIDs[strID]])
                    hashIDs[strID] = iLine
